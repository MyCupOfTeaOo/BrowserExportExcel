...
import { englishAlphabet } from '../../utils/utils';

/* eslint no-param-reassign: 0 */

export default function({ sheetName, columns, rows, filename, title }) {
  const workbook = new Excel.Workbook();
  workbook.creator = 'tea github:https://github.com/tz310200/BrowserExportExcel';
  workbook.lastModifiedBy = 'tea github:https://github.com/tz310200/BrowserExportExcel';
  ...
  allMerge.forEach(coordinate => {
    ...
    worksheet.getCell(englishAlphabet[coordinate[1] - 1] + coordinate[0]).alignment = {
      vertical: 'middle',
      horizontal: 'center',
    };
  }); // 合并表头
...
function getEveryColumns(
  ...
  coordinate[2] =
    forwardBottom + 1 - titleLength === depth
      ? forwardBottom + 1
      : forwardBottom + depth - downDepth; // row2
  coordinate[3] = forwardLeft + colObj.leafNodeNum; // col2
  if (coordinate[0] !== coordinate[2] || coordinate[1] !== coordinate[3]) allMerge.push(coordinate);
...
function getDepth(depth, element) {
  if (Array.isArray(element.children))
    return Math.max(depth, ...element.children.map(element2 => getDepth(depth + 1, element2)));
  return depth;
}
